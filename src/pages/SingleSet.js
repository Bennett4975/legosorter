            ...
            <ul>
                {parts.map((item) => (
                    <li key={`${item.part.part_num}-${item.color.id}`}>
                                   ...
                                   {ownedData && (
                                    <>
                                      <h5>{ownedData.parts[`${item.part.part_num}-${item.color.id}`].owned}/{ownedData.parts[`${item.part.part_num}-${item.color.id}`].quantity}</h5>
                                      <button onClick={() => handleContribute(ownedData.set_num, `${item.part.part_num}-${item.color.id}`)}
                                        style={{ marginLeft: 12 }}
                                      >
                                      Add 1
                                      </button>
                                      <button onClick={() => handleRemoval(ownedData.set_num, `${item.part.part_num}-${item.color.id}`)}
                                        style={{ marginLeft: 12 }}
                                      >
                                      Remove 1
                                      </button>
